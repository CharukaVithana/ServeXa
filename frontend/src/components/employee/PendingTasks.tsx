...
const PendingTasks: React.FC = () => {
  ...
  const fetchPendingTasks = async () => {
    setLoading(true);
    try {
      const pendingTasks = await employeeService.getPendingTasks();
  ...
  const handleStartTask = async (taskId: string) => {
    try {
      await employeeService.startTask(taskId);
      // Navigate to Current Tasks tab
      navigate("/employee/current-tasks");
    } catch (err) {
